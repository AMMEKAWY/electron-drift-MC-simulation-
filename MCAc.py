...
while l != len(E):
	...
	while x[-1] < d:
		...
		#xf, yf, zf = 0,0,0
		xd1,yd1,zd1=(xd[-2]), (xd[-2]) ,(xd[-2])
		
		xd2,yd2,zd2= (xd[-1]) ,(yd[-1]) ,(zd[-1]) 
		vxf,vyf,vzf=(vx[-1]) ,(vy[-1]), (vz[-1])
		...
		vd.append(np.sqrt(vx[-1]**2+vy[-1]**2+vz[-1]**2))	
		
		kl=np.sqrt(abs(xd[-1])**2+abs(yd[-1])**2+abs(zd[-1])**2)
		
		v=np.append(v,np.sqrt(vx[-1]**2))
		
		ttau=ttau+(me*abs(v[-1]-v[-2])/(electroncharge*E[-1]))
		
		vd.append(abs(d/ttau))
	...
	sigmat=hest(men)+tex(2, men)+tion(2, men)
	
	print(tion(2, men)/sigmat/d*0.01)
	
	#print(round(E[l]/100), np.sqrt(2*(np.mean(energy[-1])*electroncharge)/me)*10**-7,"	", inelaio[-1])
	print(round(E[l]/100),"		",(vd[-1])*10**-7,"	", (inelaio[-1])/d*0.01)
