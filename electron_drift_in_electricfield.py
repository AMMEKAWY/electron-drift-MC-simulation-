...
E=np.linspace(1000,1.5*10**7, 10)
...
def mainfunc(ener, ionE, excE , ln, elas, inela, inelaion, ioni, distance):
	...
	if (ener[-1]>=0):
		...
		if(ener[-1]<ionE[ln]):
			...
			if (n_p_ions(ener[-1]-ionE[ln],excE[ln],ionE[ln])<0.5):
...
while l != len(E):
	...
	while x[-1] < d:
		...
		if (energy[-1]> e_exc[2] and energy[-1] < e_ion[2]):
			...
			energy.append(energy[-1]-e_exc[2])
		
		elif(energy[-1] > e_ion[2]):
		
			energy.append(energy[-1]-e_ion[2])
			inelaio.append(inelaio[-1]+1)
			inelaio.pop(0)
		else:
